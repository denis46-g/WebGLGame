var lightLocation = [10.0, 13.0, -15.0];
...
window.onload = function () {
    ...
    var imgs = [];
    var objs = [];
    ["Field", "tank", "teasure", "scorpion", "fish"].forEach((name) =>
    {
...
function render(objs, gl, program, first) {
    ...
    draw_elem(objs[1], gl, program, x_location, mainZkoef + z_location, -0.2, 0.32, first, 1, true, true);
...
function draw_elem(obj, gl, program, x, z, y, k, first, i, first_in_item, isMain) {
    ...
    if (!isMain) {
        ...
        mat4.rotateY(Invmat, Invmat, angle);
    }
    mat4.translate(mvMatrix, mvMatrix, [0, 0, (1 / k) * mainZkoef]);

    //!!!
    mat4.translate(mvMatrix, mvMatrix, [(1 / k) * (x - x_location), -(1.25 / k) + y, (1 / k) * (z_location - z)]);
    //mat4.translate(Invmat, Invmat, [(1 / k) * (x - x_location), -(1.25 / k) + y, (1 / k) * (z_location - z)]);
